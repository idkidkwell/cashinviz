...
export const MIXER_FACTORY_ADDRESSES: Record<number, `0x${string}`> = {
  // Sepolia testnet — NOT YET DEPLOYED. The first deploy attempt
  // failed because the Noir-generated `HonkVerifier` is 27,750 bytes
  // and Sepolia / mainnet enforce EIP-170 (24,576-byte cap on
  // contract code). A Groth16 migration is in progress to bring the
  // verifier under the limit. See SECURITY.md for the production
  // blockers.
  11155111: "0x0000000000000000000000000000000000000000",
  // Mainnet — pending audit AND the EIP-170 fix above.
  1: "0x0000000000000000000000000000000000000000",
};

// `DEPLOYED_CONTRACTS` will return here once the Groth16-verifier
// branch lands and the deploy succeeds on Sepolia. Until then there's
// no chain-specific configuration to expose; the wagmi.ts call sites
// already handle this by routing through `getMixerFactoryAddress()`,
// which throws `MixerNotDeployedError` when the factory is the zero
// address.
